// Package usersclient resolves billing identity from iag-users.
package usersclient
...
func (c *Client) ListBillingIdentities(ctx context.Context, orgID string) ([]BillingIdentity, error) {
	if !c.Enabled() {
		return nil, ErrNotConfigured
	}
	var out struct {
		Data []BillingIdentity `json:"data"`
	}
	path := fmt.Sprintf("/v1/orgs/%s/billing-identities", orgID)
